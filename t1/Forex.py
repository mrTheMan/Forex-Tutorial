...
def patternrecognition():
    ...
    if patFound == 1:
        ...
        print(predarray)
        predictedAverage = functools.reduce(lambda x, y: x+y, predarray)/len(predarray)

        print(predictedAverage)
        if(predictedAverage < 0):
...
while toWhat < dataLength:
    ...
    moveOn = input('Press ENTER to contenue...')
    samps += 1
    toWhat += 1

    if len(accuracyAr) > 0:
        accuracyAverage = functools.reduce(lambda x, y: x+y, accuracyAr)/len(accuracyAr)
        print('Backtestet accuracy is ', str(accuracyAverage), ' After ', samps, ' samples')
